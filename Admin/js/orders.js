...
function acceptOrder(oId){
    ...
    var dateAccept = d.toDateString() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
...
function rejectOrder(oId){
    var k;
    var d = new Date();
    var dateReject = d.toDateString() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
    ...
    for(k=0; k<totalProducts-1; k++){
        ...
        if(arrDet[k].ID === oId){
            ...
            firebase.database().ref("UserOrders/").child(ouserIdArr[k]).child("Placed").child(oId).remove();
            console.log("Order status: Rejected");
